...
namespace AYMDatingCore.BLL.Repositories
{
    public class EmailTBLRepository : GenericRepository<EmailTBL>, IEmailTBLRepository
    {
        ...
        public EmailTBLRepository(AymanDatingCoreDbContext context, IConfiguration configuration) :base(context)
        {
            _context = context;
        ...
        public async Task SendEmail(EmailTBL email)
        {
            ...
            client.Credentials = new NetworkCredential(configuration["AymanDatingCore.Pl.SendingEmail"], configuration["AymanDatingCore.Pl.PWSendingEmail"]);

            client.Send(configuration["AymanDatingCore.Pl.SendingEmail"], email.To, email.Subject, email.Body);
        ...
        public async Task SendEmailAsync(EmailTBL emails)
        {
            ...
            email.From.Add(MailboxAddress.Parse(configuration["AymanDatingCore.Pl.SendingEmail"]));
            ...
            await smtp.AuthenticateAsync(configuration["AymanDatingCore.Pl.SendingEmail"], configuration["AymanDatingCore.Pl.PWSendingEmail"]);
        ...
        public async Task SendEmailAsync(EmailTBL emails, int SecondType = 0, List<string>? ccEmails = null)
        {
            ...
            {
                Port = 587,
                Credentials = new NetworkCredential(emails.From, "xwivnfzebmhthhsb"),
                EnableSsl = true,
            };

            var mail = new MailMessage
            {
                From = new MailAddress(emails.From, "Dating House"),
